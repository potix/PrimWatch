...
static int group_foreach_cb(void *foreach_arg, const char *path, bson_iterator *itr) {
	...
	key = bson_iterator_key(itr);
        printf("group.%s\n", key);
	ASSERT(bson_helper_itr_get_long(itr, &l, "maxForwardRecords", config, "defaultMaxForwardRecords") == 0);
        ASSERT(l == 3);
	ASSERT(bson_helper_itr_get_long(itr, &l, "maxReverseRecords", config, "defaultMaxReverseRecords") == 0);
        ASSERT(l == 1);
	snprintf(p, sizeof(p), "%s.maxForwardRecords", key);
	ASSERT(bson_helper_itr_get_long(itr, &l, p, config, "defaultMaxForwardRecords") == 0);
        ASSERT(l == 3);
        printf("group.%s.maxForwardRecords - %lld\n", key, (long long)l);
	snprintf(p, sizeof(p), "%s.maxReverseRecords", key);
	ASSERT(bson_helper_itr_get_long(itr, &l, p, config, "defaultMaxReverseRecords") == 0);
        ASSERT(l == 1);
        printf("group.%s.maxReverseRecords - %lld\n", key, (long long)l);
